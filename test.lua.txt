...
G2L["2"]["Size"] = UDim2.new(0, 300, 0, 380);
G2L["2"]["Position"] = UDim2.new(0.5, 2, 0.5, 6);
...
G2L["4"]["Color"] = Color3.fromRGB(61, 61, 71);


-- StarterGui.Lurk.Frame.top
G2L["5"] = Instance.new("Frame", G2L["2"]);
G2L["5"]["BorderSizePixel"] = 0;
G2L["5"]["BackgroundColor3"] = Color3.fromRGB(21, 21, 26);
G2L["5"]["Size"] = UDim2.new(1, 0, 0, 35);
G2L["5"]["Name"] = [[top]];
...
G2L["7"]["TextColor3"] = Color3.fromRGB(241, 241, 241);
G2L["7"]["BackgroundTransparency"] = 1;
G2L["7"]["Size"] = UDim2.new(1, -50, 1, 0);
G2L["7"]["Text"] = [[LURK HACK]];
G2L["7"]["Position"] = UDim2.new(0, 12, 0, 0);
...
G2L["8"]["BackgroundColor3"] = Color3.fromRGB(201, 51, 51);
G2L["8"]["FontFace"] = Font.new([[rbxasset://fonts/families/GothamSSm.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
G2L["8"]["Size"] = UDim2.new(0, 28, 0, 28);
G2L["8"]["Text"] = [[×]];
G2L["8"]["Name"] = [[Close]];
G2L["8"]["Position"] = UDim2.new(1, -33, 0, 4);
...
G2L["b"]["Size"] = UDim2.new(1, -20, 0, 32);
G2L["b"]["Position"] = UDim2.new(0, 10, 0, 40);
G2L["b"]["Name"] = [[tabs]];
...
G2L["d"]["BackgroundColor3"] = Color3.fromRGB(0, 121, 216);
...
G2L["f"]["BackgroundColor3"] = Color3.fromRGB(41, 41, 51);
...
G2L["11"]["TextSize"] = 14;
G2L["11"]["TextColor3"] = Color3.fromRGB(241, 241, 241);
G2L["11"]["BackgroundColor3"] = Color3.fromRGB(41, 41, 51);
...
G2L["13"]["CanvasSize"] = UDim2.new(0, 0, 0, 192);
G2L["13"]["Size"] = UDim2.new(1, -20, 1, -90);
G2L["13"]["ScrollBarImageColor3"] = Color3.fromRGB(0, 171, 255);
G2L["13"]["Position"] = UDim2.new(0, 10, 0, 77);
...
local script = G2L["1d"];
	...
	local function animateToggle(state)
		...
		local thumbTween = tweenService:Create(sliderThumb, TweenInfo.new(0.2), {Position = target.Position})
		local buttonTween = tweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = target.Color})
		thumbTween:Play()
		buttonTween:Play()
	end
			...
			for _, player in pairs(Players:GetPlayers()) do
				if player ~= localPlayer then
					...
					player.CharacterAdded:Connect(function(char)
						if isEnabled then applyHighlight(char) end
					end)
				end
			end
		else
			...
			for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
				...
				if character then
					...
					if humanoidRootPart then
						local esp = humanoidRootPart:FindFirstChild("ESP_"..player.Name)
						if esp then
							esp:Destroy()
						end
					end
...
local script = G2L["4f"];
	...
	-- Оптимизированный список состояний оглушения
	...
	local function setupAntiStun()
		...
		table.insert(antiStunConnections, localPlayer.CharacterAdded:Connect(function(newChar)
			task.wait(0.5) -- Даем время на полную загрузку
			if isEnabled then
				setupAntiStun()
			end
		end))
...
local script = G2L["56"];
	...
	local function setupAntiAfk()
		...
		local function applyAntiAfk()
			...
			-- Periodic subtle movement (every 30 seconds)
			antiAfkConnection = game:GetService("RunService").Heartbeat:Connect(function(step)
				if tick() % 30 < step then
					humanoid:Move(Vector3.new(math.random(-1,1), 0, math.random(-1,1)))
				end
			end)
	
			-- Use Idled event to simulate user input
